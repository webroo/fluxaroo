...
function createContainer(component, options) {
  ...
  return React.createClass({
    getState: function() {
      ...
      // Copy any store props onto the new state object
      if (options.storeProps) {
        var storeProps = options.storeProps(this.props);
        for (key in storeProps) {
          state[key] = storeProps[key];
        }
      }

      // Copy any action handlers onto the new state object
      if (options.actionProps) {
        var wrappedActionProps = wrapActionProps(dispatcher, options.actionProps(this.props));
        for (key in wrappedActionProps) {
          state[key] = wrappedActionProps[key];
        }
      }
